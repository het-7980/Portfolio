...
export function renderEducation(): string {
  const items = education
    .map(
      (entry, index) => `
        <li class="timeline__item" data-reveal="right" data-reveal-index="${index}">
          <span class="timeline__dot" aria-hidden="true"><span class="timeline__ping"></span>${icon('cap')}</span>
          <div class="timeline__card" data-spotlight>
    ...
    <section class="section section--alt" id="education" aria-labelledby="education-title">
      <div class="container container--narrow">
        <header class="section__head" data-reveal>
          ...
          <h2 class="section__title" id="education-title">${words('Academic background')}</h2>
        </header>

        <ol class="timeline" data-timeline>${items}</ol>
      </div>
    </section>
  `;
}
